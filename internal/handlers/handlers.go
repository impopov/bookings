...
import (
	...
	"github.com/ivanpopov/bookings/internal/helpers"
	"github.com/ivanpopov/bookings/internal/models"
	"github.com/ivanpopov/bookings/internal/render"
	"github.com/ivanpopov/bookings/internal/repository"
	"github.com/ivanpopov/bookings/internal/repository/dbrepo"
	"log"
	"net/http"
	"strconv"
	"strings"
...
// Oceans renders the room page
func (m *Repository) Oceans(w http.ResponseWriter, r *http.Request) {
	render.Template(w, r, "oceans.page.gohtml", &models.TemplateData{})
}

// Dudes renders the room page
func (m *Repository) Dudes(w http.ResponseWriter, r *http.Request) {
	render.Template(w, r, "dudes.page.gohtml", &models.TemplateData{})
}
...
func (m *Repository) AdminReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	for _, x := range rooms {
		...
		restrictions, err := m.DB.GerRestrictionsForRoomByDate(x.ID, firstOfMonth, lastOfMonth)
		if err != nil {
			helpers.ServerError(w, err)
			return
		}
...
func (m *Repository) AdminPostReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	for _, x := range rooms {
		// get the block map from session. loop for map, if we have entry in the map
		// that does not exist in out posted data, and the restriction id > 0 thant in is a block
		// we need to remove
		...
		for name, value := range curMap {
			...
			if val, ok := curMap[name]; ok {
				...
				// the rest a just placeholders for days without blocks
				if val > 0 {
