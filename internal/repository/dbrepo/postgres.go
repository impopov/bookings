...
// UpdateUser updates a user into database
...
// UpdateReservation updates a reservation into database
...
// GerRestrictionsForRoomByDate returns restrictions for a room by date range
func (m *postgresDBRepo) GerRestrictionsForRoomByDate(roomID int, start, end time.Time) ([]models.RoomRestriction, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
